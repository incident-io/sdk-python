...
T = TypeVar("T", bound="MaintenanceWindowEscalationTargetV1")
...
class MaintenanceWindowEscalationTargetV1:
    ...
    Attributes:
        escalation_paths (EngineParamBindingV2 | Unset):  Example: {'array_value': [{'label': 'Lawrence Jones',
            'literal': 'SEV123', 'reference': 'incident.severity'}], 'value': {'label': 'Lawrence Jones', 'literal':
            'SEV123', 'reference': 'incident.severity'}}.
        users (EngineParamBindingV2 | Unset):  Example: {'array_value': [{'label': 'Lawrence Jones', 'literal':
            'SEV123', 'reference': 'incident.severity'}], 'value': {'label': 'Lawrence Jones', 'literal': 'SEV123',
            'reference': 'incident.severity'}}.
    """

    escalation_paths: EngineParamBindingV2 | Unset = UNSET
    users: EngineParamBindingV2 | Unset = UNSET
    ...
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        ...
        escalation_paths: EngineParamBindingV2 | Unset
        if isinstance(_escalation_paths, Unset):
            escalation_paths = UNSET
        else:
            escalation_paths = EngineParamBindingV2.from_dict(_escalation_paths)

        _users = d.pop("users", UNSET)
        users: EngineParamBindingV2 | Unset
        if isinstance(_users, Unset):
            users = UNSET
        else:
            users = EngineParamBindingV2.from_dict(_users)

        maintenance_window_escalation_target_v1 = cls(
            escalation_paths=escalation_paths,
            users=users,
        )

        maintenance_window_escalation_target_v1.additional_properties = d
        return maintenance_window_escalation_target_v1
