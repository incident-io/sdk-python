...
if TYPE_CHECKING:
    from ..models.alert_route_channel_target_payload_v3 import (
        AlertRouteChannelTargetPayloadV3,
    )
    from ..models.condition_group_payload_v3 import ConditionGroupPayloadV3


T = TypeVar("T", bound="AlertMessageDestinationPayloadV3")


@_attrs_define(kw_only=True)
class AlertMessageDestinationPayloadV3:
    """
    Example:
        {'condition_groups': [{'conditions': [{'operation': 'one_of', 'param_bindings': [{'array_value': [{'literal':
            ...
            'incident.severity'}}], 'subject': 'alert.priority'}]}], 'ms_teams_targets': {'binding': {'array_value':
            [{'literal': 'SEV123', 'reference': 'incident.severity'}], 'value': {'literal': 'SEV123', 'reference':
            'incident.severity'}}, 'channel_visibility': 'public', 'group_alerts_summary': False}, 'slack_targets':
    ...
    Attributes:
        condition_groups (list[ConditionGroupPayloadV3]): The conditions that must be met for this channel config to be
        ...
        ms_teams_targets (AlertRouteChannelTargetPayloadV3 | Unset):  Example: {'binding': {'array_value': [{'literal':
            'SEV123', 'reference': 'incident.severity'}], 'value': {'literal': 'SEV123', 'reference': 'incident.severity'}},
            'channel_visibility': 'public', 'group_alerts_summary': False}.
        slack_targets (AlertRouteChannelTargetPayloadV3 | Unset):  Example: {'binding': {'array_value': [{'literal':
            'SEV123', 'reference': 'incident.severity'}], 'value': {'literal': 'SEV123', 'reference': 'incident.severity'}},
            'channel_visibility': 'public', 'group_alerts_summary': False}.
    """

    condition_groups: list[ConditionGroupPayloadV3]
    ms_teams_targets: AlertRouteChannelTargetPayloadV3 | Unset = UNSET
    slack_targets: AlertRouteChannelTargetPayloadV3 | Unset = UNSET
    ...
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        ...
        ms_teams_targets: AlertRouteChannelTargetPayloadV3 | Unset
        if isinstance(_ms_teams_targets, Unset):
            ms_teams_targets = UNSET
        else:
            ms_teams_targets = AlertRouteChannelTargetPayloadV3.from_dict(
                _ms_teams_targets
            )

        _slack_targets = d.pop("slack_targets", UNSET)
        slack_targets: AlertRouteChannelTargetPayloadV3 | Unset
        if isinstance(_slack_targets, Unset):
            slack_targets = UNSET
        else:
            slack_targets = AlertRouteChannelTargetPayloadV3.from_dict(_slack_targets)

        alert_message_destination_payload_v3 = cls(
            condition_groups=condition_groups,
            ms_teams_targets=ms_teams_targets,
            slack_targets=slack_targets,
        )

        alert_message_destination_payload_v3.additional_properties = d
        return alert_message_destination_payload_v3
