...
if TYPE_CHECKING:
    from ..models.alert_route_channel_target_v2 import AlertRouteChannelTargetV2
    from ..models.condition_group_v2 import ConditionGroupV2


T = TypeVar("T", bound="AlertRouteChannelConfigV2")
...
class AlertRouteChannelConfigV2:
    ...
    Attributes:
        condition_groups (list[ConditionGroupV2]): The conditions that must be met for this channel config to be used
        ...
        ms_teams_targets (AlertRouteChannelTargetV2 | Unset):  Example: {'binding': {'array_value': [{'label': 'Lawrence
        ...
        slack_targets (AlertRouteChannelTargetV2 | Unset):  Example: {'binding': {'array_value': [{'label': 'Lawrence
            Jones', 'literal': 'SEV123', 'reference': 'incident.severity'}], 'value': {'label': 'Lawrence Jones', 'literal':
            'SEV123', 'reference': 'incident.severity'}}, 'channel_visibility': 'abc123'}.
    """

    condition_groups: list[ConditionGroupV2]
    ms_teams_targets: AlertRouteChannelTargetV2 | Unset = UNSET
    slack_targets: AlertRouteChannelTargetV2 | Unset = UNSET
    ...
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        from ..models.alert_route_channel_target_v2 import (
        ...
        ms_teams_targets: AlertRouteChannelTargetV2 | Unset
        if isinstance(_ms_teams_targets, Unset):
            ms_teams_targets = UNSET
        else:
            ms_teams_targets = AlertRouteChannelTargetV2.from_dict(_ms_teams_targets)

        _slack_targets = d.pop("slack_targets", UNSET)
        slack_targets: AlertRouteChannelTargetV2 | Unset
        if isinstance(_slack_targets, Unset):
            slack_targets = UNSET
        else:
            slack_targets = AlertRouteChannelTargetV2.from_dict(_slack_targets)

        alert_route_channel_config_v2 = cls(
            condition_groups=condition_groups,
            ms_teams_targets=ms_teams_targets,
            slack_targets=slack_targets,
        )

        alert_route_channel_config_v2.additional_properties = d
        return alert_route_channel_config_v2
