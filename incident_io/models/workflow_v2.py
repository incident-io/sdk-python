...
from ..models.workflow_v2_runs_on_incidents import WorkflowV2RunsOnIncidents
from ..models.workflow_v2_state import WorkflowV2State
...
if TYPE_CHECKING:
    ...
    from ..models.step_config_v2 import StepConfigV2
...
class WorkflowV2:
    ...
    Attributes:
        ...
        private_incident_scope (WorkflowV2PrivateIncidentScope): Which private incidents this workflow acts on: every
            private incident (all), those an owning team can see (owning_teams), or none Example: owning_teams.
        runs_on_incident_modes (list[WorkflowV2RunsOnIncidentModesItem]): Which incident modes should this workflow run
            on? By default, workflows only run on standard incidents, but can also be configured to run on test and
            retrospective incidents. Example: ['standard', 'test', 'retrospective'].
        runs_on_incidents (WorkflowV2RunsOnIncidents): Which incidents should the workflow be applied to? Example:
            newly_created.
        state (WorkflowV2State): What state this workflow is in Example: active.
        steps (list[StepConfigV2]): Steps that are executed as part of the workflow Example: [{'for_each': 'abc123',
    ...
    private_incident_scope: WorkflowV2PrivateIncidentScope
    runs_on_incident_modes: list[WorkflowV2RunsOnIncidentModesItem]
    runs_on_incidents: WorkflowV2RunsOnIncidents
    state: WorkflowV2State
    steps: list[StepConfigV2]
    trigger: TriggerSlimV2
    version: int
    delay: WorkflowDelayV2 | Unset = UNSET
    folder: str | Unset = UNSET
    form_fields: list[WorkflowFormFieldV2] | Unset = UNSET
    ...
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        ...
        for runs_on_incident_modes_item_data in _runs_on_incident_modes:
            runs_on_incident_modes_item = WorkflowV2RunsOnIncidentModesItem(
                runs_on_incident_modes_item_data
            )

            runs_on_incident_modes.append(runs_on_incident_modes_item)

        runs_on_incidents = WorkflowV2RunsOnIncidents(d.pop("runs_on_incidents"))

        state = WorkflowV2State(d.pop("state"))

        steps = []
        _steps = d.pop("steps")
        for steps_item_data in _steps:
            steps_item = StepConfigV2.from_dict(steps_item_data)
        ...
        if _form_fields is not UNSET:
            ...
            for form_fields_item_data in _form_fields:
                form_fields_item = WorkflowFormFieldV2.from_dict(form_fields_item_data)
        ...
        workflow_v2.additional_properties = d
        return workflow_v2
