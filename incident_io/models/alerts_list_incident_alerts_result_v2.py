...
T = TypeVar("T", bound="AlertsListIncidentAlertsResultV2")
...
class AlertsListIncidentAlertsResultV2:
    """
    Example:
        {'incident_alerts': [{'alert': {'alert_group_ids': ['01GW2G3V0S59R238FAHPDS1R66'], 'alert_source_id':
            ...
            sad, and we don't know why yet.", 'visibility': 'public'}}], 'pagination_meta': {'after':
            '01FCNDV6P870EA6S7TK1DSYDG0', 'page_size': 25}}

    Attributes:
        incident_alerts (list[IncidentAlertV2]):  Example: [{'alert': {'alert_group_ids':
    ...
    incident_alerts: list[IncidentAlertV2]
    pagination_meta: PaginationMetaResultV2
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        incident_alerts = []
        for incident_alerts_item_data in self.incident_alerts:
            incident_alerts_item = incident_alerts_item_data.to_dict()
            incident_alerts.append(incident_alerts_item)
    ...
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        ...
        d = dict(src_dict)
        incident_alerts = []
        _incident_alerts = d.pop("incident_alerts")
        for incident_alerts_item_data in _incident_alerts:
            incident_alerts_item = IncidentAlertV2.from_dict(incident_alerts_item_data)

            incident_alerts.append(incident_alerts_item)

        pagination_meta = PaginationMetaResultV2.from_dict(d.pop("pagination_meta"))

        alerts_list_incident_alerts_result_v2 = cls(
            incident_alerts=incident_alerts,
            pagination_meta=pagination_meta,
        )

        alerts_list_incident_alerts_result_v2.additional_properties = d
        return alerts_list_incident_alerts_result_v2
