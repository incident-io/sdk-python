...
T = TypeVar("T", bound="EscalationPathTemplateNodeIfElsePayloadV2")
...
class EscalationPathTemplateNodeIfElsePayloadV2:
    ...
    Attributes:
        else_path (list[EscalationPathTemplateNodePayloadV2]): The nodes taken if the condition is not met Example:
            ...
            'active', 'time_to_ack_seconds': 1800, 'time_to_ack_weekday_interval_config_id': '01FCNDV6P870EA6S7TK1DSYDG0'},
            'repeat': {'repeat_times': 3, 'to_node': '01FCNDV6P870EA6S7TK1DSYDG0'}, 'type': 'if_else'}].
        then_path (list[EscalationPathTemplateNodePayloadV2]): The nodes taken if the condition is met Example:
            [{'delay': {'delay_interval_condition': 'active', 'delay_seconds': 300, 'delay_weekday_interval_config_id':
            '01FCNDV6P870EA6S7TK1DSYDG0'}, 'escalation_path': {'escalation_path_id': '01FCNDV6P870EA6S7TK1DSYDG0'}, 'id':
            '01FCNDV6P870EA6S7TK1DSYDG0', 'if_else': {'conditions': [{'operation': 'one_of', 'param_bindings':
            [{'array_value': [{'literal': 'SEV123', 'reference': 'incident.severity'}], 'value': {'literal': 'SEV123',
            ...
            'active', 'time_to_ack_seconds': 1800, 'time_to_ack_weekday_interval_config_id': '01FCNDV6P870EA6S7TK1DSYDG0'},
            'repeat': {'repeat_times': 3, 'to_node': '01FCNDV6P870EA6S7TK1DSYDG0'}, 'type': 'if_else'}].
        conditions (list[ConditionPayloadV2] | Unset): The condition that defines which branch to take Example:
            [{'operation': 'one_of', 'param_bindings': [{'array_value': [{'literal': 'SEV123', 'reference':
            'incident.severity'}], 'value': {'literal': 'SEV123', 'reference': 'incident.severity'}}], 'subject':
            'incident.severity'}].
    """

    else_path: list[EscalationPathTemplateNodePayloadV2]
    then_path: list[EscalationPathTemplateNodePayloadV2]
    conditions: list[ConditionPayloadV2] | Unset = UNSET
    ...
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        ...
        for else_path_item_data in _else_path:
            else_path_item = EscalationPathTemplateNodePayloadV2.from_dict(
                else_path_item_data
            )
        ...
        for then_path_item_data in _then_path:
            then_path_item = EscalationPathTemplateNodePayloadV2.from_dict(
                then_path_item_data
            )
        ...
        escalation_path_template_node_if_else_payload_v2 = cls(
            else_path=else_path,
            then_path=then_path,
            conditions=conditions,
        )

        escalation_path_template_node_if_else_payload_v2.additional_properties = d
        return escalation_path_template_node_if_else_payload_v2
