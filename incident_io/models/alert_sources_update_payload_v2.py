...
T = TypeVar("T", bound="AlertSourcesUpdatePayloadV2")
...
class AlertSourcesUpdatePayloadV2:
    ...
    Example:
        {'auto_resolve_incident_alerts': False, 'auto_resolve_timeout_minutes': 1, 'azure_devops_options':
            {'project_ids': ['01GBSQF3FHF7FWZQNWGHAVQ804', 'ba038695-f5f7-4490-a9ea-bf4ab3cbf483']}, 'disabled': False,
            'email_options': {'redactions': ['credit_card_numbers'], 'transform_expression': "return {\\n  title:
            $.subject,\\n  description: $.text,\\n  status: $.subject.startsWith('[RESOLVED]') ? 'resolved' : 'firing',\\n
            deduplication_key: $.header_message_id,\\n}"}, 'filter_condition_groups': [{'conditions': [{'operation':
            ...
            'rate_limit_sharding': {'rate_limit_shard_key_path': '$.priority'}, 'template': {'attributes':
    ...
    Attributes:
        ...
        disabled (bool | Unset): For heartbeat sources, set to true to disable monitoring Example: False.
        ...
        filter_condition_groups (list[ConditionGroupPayloadV2] | Unset): Conditions an incoming event must match to be
            ingested from this source, evaluated against the event's payload and this source's expressions. When empty,
            everything is ingested; otherwise a firing event that doesn't match is dropped and never creates or updates an
            alert. Resolve events are never filtered. Omit to leave unchanged. Example: [{'conditions': [{'operation':
            'one_of', 'param_bindings': [{'array_value': [{'literal': 'SEV123', 'reference': 'incident.severity'}], 'value':
        ...
        fixed_team_id (str | Unset): Fix the team every alert from this source is attributed to. While set, the team
            attribute is managed from this field: a team binding sent in the template is ignored. Omit to leave unchanged;
            set to an empty string to clear it, making the team attribute editable again. Example:
    ...
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        ...
        alert_sources_update_payload_v2 = cls(
            name=name,
            template=template,
        ...
        alert_sources_update_payload_v2.additional_properties = d
        return alert_sources_update_payload_v2
