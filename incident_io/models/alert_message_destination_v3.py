...
if TYPE_CHECKING:
    from ..models.alert_route_channel_target_v3 import AlertRouteChannelTargetV3
    from ..models.condition_group_v3 import ConditionGroupV3


T = TypeVar("T", bound="AlertMessageDestinationV3")
...
class AlertMessageDestinationV3:
    ...
    Attributes:
        condition_groups (list[ConditionGroupV3]): The conditions that must be met for this channel config to be used
            ...
            'alert.priority'}}]}].
        ms_teams_targets (AlertRouteChannelTargetV3 | Unset):  Example: {'binding': {'array_value': [{'literal':
            'SEV123', 'reference': 'incident.severity'}], 'value': {'literal': 'SEV123', 'reference': 'incident.severity'}},
            'channel_visibility': 'abc123', 'group_alerts_summary': False}.
        slack_targets (AlertRouteChannelTargetV3 | Unset):  Example: {'binding': {'array_value': [{'literal': 'SEV123',
            'reference': 'incident.severity'}], 'value': {'literal': 'SEV123', 'reference': 'incident.severity'}},
            'channel_visibility': 'abc123', 'group_alerts_summary': False}.
    """

    condition_groups: list[ConditionGroupV3]
    ms_teams_targets: AlertRouteChannelTargetV3 | Unset = UNSET
    slack_targets: AlertRouteChannelTargetV3 | Unset = UNSET
    ...
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        from ..models.alert_route_channel_target_v3 import (
        ...
        ms_teams_targets: AlertRouteChannelTargetV3 | Unset
        if isinstance(_ms_teams_targets, Unset):
            ms_teams_targets = UNSET
        else:
            ms_teams_targets = AlertRouteChannelTargetV3.from_dict(_ms_teams_targets)

        _slack_targets = d.pop("slack_targets", UNSET)
        slack_targets: AlertRouteChannelTargetV3 | Unset
        if isinstance(_slack_targets, Unset):
            slack_targets = UNSET
        else:
            slack_targets = AlertRouteChannelTargetV3.from_dict(_slack_targets)

        alert_message_destination_v3 = cls(
            condition_groups=condition_groups,
            ms_teams_targets=ms_teams_targets,
            slack_targets=slack_targets,
        )

        alert_message_destination_v3.additional_properties = d
        return alert_message_destination_v3
