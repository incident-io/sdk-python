...
from ..models.policies_update_payload_v2_policy_type import (
    PoliciesUpdatePayloadV2PolicyType,
)
from ..models.policies_update_payload_v2_status import PoliciesUpdatePayloadV2Status
...
if TYPE_CHECKING:
    ...
    from ..models.policy_assignment_rules_payload_v2 import (
        PolicyAssignmentRulesPayloadV2,
    )
    from ..models.policy_debrief_payload_v2 import PolicyDebriefPayloadV2
    from ..models.policy_follow_up_payload_v2 import PolicyFollowUpPayloadV2
    from ..models.policy_on_call_readiness_v2 import PolicyOnCallReadinessV2
    from ..models.policy_post_mortem_payload_v2 import PolicyPostMortemPayloadV2
    from ..models.policy_schedule_v2 import PolicyScheduleV2


T = TypeVar("T", bound="PoliciesUpdatePayloadV2")


@_attrs_define(kw_only=True)
class PoliciesUpdatePayloadV2:
    """
    Example:
        {'assignment_rules': {'bindings': [{'array_value': [{'literal': 'SEV123', 'reference': 'incident.severity'}],
            'value': {'literal': 'SEV123', 'reference': 'incident.severity'}}], 'reminder_cadence_after': {'interval':
            'daily'}, 'reminder_cadence_before': {'interval': 'daily'}, 'reminder_detected_date_offset_hours': [0, 48],
            'reminder_due_date_offset_hours': [-24, 0, 24]}, 'conditions': [{'conditions': [{'operation': 'one_of',
            'param_bindings': [{'array_value': [{'literal': 'SEV123', 'reference': 'incident.severity'}], 'value':
            {'literal': 'SEV123', 'reference': 'incident.severity'}}], 'subject': 'incident.severity'}]}], 'debrief':
            ...
            'run_on_private_incidents': False}, 'description': 'All critical incidents must export follow-ups to an external
            issue tracker before 7 days has passed since closure.', 'expressions': [{'else_branch': {'result':
            ...
            'root_reference': 'incident.status'}], 'follow_up': {'due_date_config': {'applies_from':
            ...
            'incident.severity'}]}], 'run_on_private_incidents': False}, 'name': 'Critical incidents must export follow-
            ups', 'on_call_readiness': {'enforcement': 'advisory', 'high_urgency': [{'max_delay_seconds': 300,
            ...
            'calculation_timezone': 'Europe/London', 'calculation_type': 'weekdays', 'days': {'array_value': [{'literal':
            'SEV123', 'reference': 'incident.severity'}], 'value': {'literal': 'SEV123', 'reference': 'incident.severity'}},
            'incident_timestamp_id': '01FCNDV6P870EA6S7TK1DSYDG0'}, 'requirements': [{'conditions': [{'operation': 'one_of',
            'param_bindings': [{'array_value': [{'literal': 'SEV123', 'reference': 'incident.severity'}], 'value':
            {'literal': 'SEV123', 'reference': 'incident.severity'}}], 'subject': 'incident.severity'}]}],
            'run_on_private_incidents': False}, 'schedule': {'evaluation_level': 'schedule', 'requirement_type':
            'contiguous'}, 'status': 'enabled'}

    Attributes:
        conditions (list[ConditionGroupPayloadV2]): Conditions which determine which resources are in scope for this
            policy Example: [{'conditions': [{'operation': 'one_of', 'param_bindings': [{'array_value': [{'literal':
            'SEV123', 'reference': 'incident.severity'}], 'value': {'literal': 'SEV123', 'reference':
            'incident.severity'}}], 'subject': 'incident.severity'}]}].
        description (str): Human readable description of the policy Example: All critical incidents must export follow-
            ups to an external issue tracker before 7 days has passed since closure..
        name (str): Human readable name of the policy Example: Critical incidents must export follow-ups.
        policy_type (PoliciesUpdatePayloadV2PolicyType): Type of the policy, specifying what this applies to. Cannot be
            changed after the policy is created. Example: follow_up.
        assignment_rules (PolicyAssignmentRulesPayloadV2 | Unset):  Example: {'bindings': [{'array_value': [{'literal':
            'SEV123', 'reference': 'incident.severity'}], 'value': {'literal': 'SEV123', 'reference':
        ...
        debrief (PolicyDebriefPayloadV2 | Unset): Set when policy_type is debrief. Example: {'due_date_config':
            {'applies_from': '2021-08-17T13:28:57.801578Z', 'calculation_timezone': 'Europe/London', 'calculation_type':
            'weekdays', 'days': {'array_value': [{'literal': 'SEV123', 'reference': 'incident.severity'}], 'value':
            {'literal': 'SEV123', 'reference': 'incident.severity'}}, 'incident_timestamp_id':
            '01FCNDV6P870EA6S7TK1DSYDG0'}, 'requirements': [{'conditions': [{'operation': 'one_of', 'param_bindings':
            [{'array_value': [{'literal': 'SEV123', 'reference': 'incident.severity'}], 'value': {'literal': 'SEV123',
            'reference': 'incident.severity'}}], 'subject': 'incident.severity'}]}], 'run_on_private_incidents': False}.
        expressions (list[ExpressionPayloadV2] | Unset): The expressions to use in this policy Example: [{'else_branch':
        ...
        follow_up (PolicyFollowUpPayloadV2 | Unset): Set when policy_type is follow_up. Example: {'due_date_config':
            {'applies_from': '2021-08-17T13:28:57.801578Z', 'calculation_timezone': 'Europe/London', 'calculation_type':
            'weekdays', 'days': {'array_value': [{'literal': 'SEV123', 'reference': 'incident.severity'}], 'value':
            {'literal': 'SEV123', 'reference': 'incident.severity'}}, 'incident_timestamp_id':
            '01FCNDV6P870EA6S7TK1DSYDG0'}, 'requirements': [{'conditions': [{'operation': 'one_of', 'param_bindings':
        ...
        post_mortem (PolicyPostMortemPayloadV2 | Unset): Set when policy_type is post_mortem. Example:
            {'due_date_config': {'applies_from': '2021-08-17T13:28:57.801578Z', 'calculation_timezone': 'Europe/London',
            'calculation_type': 'weekdays', 'days': {'array_value': [{'literal': 'SEV123', 'reference':
            'incident.severity'}], 'value': {'literal': 'SEV123', 'reference': 'incident.severity'}},
            'incident_timestamp_id': '01FCNDV6P870EA6S7TK1DSYDG0'}, 'requirements': [{'conditions': [{'operation': 'one_of',
        ...
        status (PoliciesUpdatePayloadV2Status | Unset): Defaults to enabled on create. Settable on update — there is no
            separate disable endpoint. Example: enabled.
    """

    conditions: list[ConditionGroupPayloadV2]
    description: str
    name: str
    policy_type: PoliciesUpdatePayloadV2PolicyType
    assignment_rules: PolicyAssignmentRulesPayloadV2 | Unset = UNSET
    debrief: PolicyDebriefPayloadV2 | Unset = UNSET
    expressions: list[ExpressionPayloadV2] | Unset = UNSET
    follow_up: PolicyFollowUpPayloadV2 | Unset = UNSET
    on_call_readiness: PolicyOnCallReadinessV2 | Unset = UNSET
    post_mortem: PolicyPostMortemPayloadV2 | Unset = UNSET
    schedule: PolicyScheduleV2 | Unset = UNSET
    status: PoliciesUpdatePayloadV2Status | Unset = UNSET
    ...
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        ...
        from ..models.policy_debrief_payload_v2 import (
            PolicyDebriefPayloadV2,
        )
        from ..models.policy_follow_up_payload_v2 import (
            PolicyFollowUpPayloadV2,
        )
        from ..models.policy_on_call_readiness_v2 import (
            PolicyOnCallReadinessV2,
        )
        from ..models.policy_post_mortem_payload_v2 import (
        ...
        for conditions_item_data in _conditions:
            conditions_item = ConditionGroupPayloadV2.from_dict(conditions_item_data)
        ...
        policy_type = PoliciesUpdatePayloadV2PolicyType(d.pop("policy_type"))

        _assignment_rules = d.pop("assignment_rules", UNSET)
        assignment_rules: PolicyAssignmentRulesPayloadV2 | Unset
        if isinstance(_assignment_rules, Unset):
            assignment_rules = UNSET
        else:
            assignment_rules = PolicyAssignmentRulesPayloadV2.from_dict(
                _assignment_rules
            )

        _debrief = d.pop("debrief", UNSET)
        debrief: PolicyDebriefPayloadV2 | Unset
        if isinstance(_debrief, Unset):
            debrief = UNSET
        else:
            debrief = PolicyDebriefPayloadV2.from_dict(_debrief)
        ...
        follow_up: PolicyFollowUpPayloadV2 | Unset
        if isinstance(_follow_up, Unset):
            follow_up = UNSET
        else:
            follow_up = PolicyFollowUpPayloadV2.from_dict(_follow_up)
        ...
        post_mortem: PolicyPostMortemPayloadV2 | Unset
        if isinstance(_post_mortem, Unset):
            post_mortem = UNSET
        else:
            post_mortem = PolicyPostMortemPayloadV2.from_dict(_post_mortem)
        ...
        status: PoliciesUpdatePayloadV2Status | Unset
        if isinstance(_status, Unset):
            status = UNSET
        else:
            status = PoliciesUpdatePayloadV2Status(_status)

        policies_update_payload_v2 = cls(
            conditions=conditions,
            description=description,
            name=name,
            policy_type=policy_type,
        ...
        policies_update_payload_v2.additional_properties = d
        return policies_update_payload_v2
